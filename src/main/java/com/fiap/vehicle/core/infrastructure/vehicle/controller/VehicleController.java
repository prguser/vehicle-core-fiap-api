package com.fiap.vehicle.core.infrastructure.vehicle.controller;

import com.fiap.vehicle.core.domain.entity.vehicle.model.Vehicle;

import com.fiap.vehicle.core.infrastructure.vehicle.dto.request.VehicleRequestDTO;
import com.fiap.vehicle.core.infrastructure.vehicle.dto.response.VehicleResponseDTO;
...
public class VehicleController {
	...
	public VehicleResponseDTO cadastrarVehicle(@RequestBody @Valid VehicleRequestDTO vehicleRequestDTO) {
		Vehicle vehicle = new Vehicle(vehicleRequestDTO.getMake(), vehicleRequestDTO.getModel(),
				vehicleRequestDTO.getColor(), vehicleRequestDTO.getMileage(), vehicleRequestDTO.getPrice(),
				vehicleRequestDTO.getStatus(), vehicleRequestDTO.getDataFabricacao(), OffsetDateTime.now());
	...
	@PutMapping("/{id}")
	@ResponseStatus(HttpStatus.OK)
	public VehicleResponseDTO atualizarVehicle(@PathVariable Long id,
	...
	@ApiResponses(value = { @ApiResponse(responseCode = "200", description = "Sucesso ao listar os Veículos",
			content = { @Content(mediaType = "application/json",
					schema = @Schema(implementation = VehicleResponseDTO.class)) }) })
	@GetMapping
	@ResponseStatus(HttpStatus.OK)
